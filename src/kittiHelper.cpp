...
int pub_data_process()   
{
    ...
    ros::Publisher pub_laser_cloud = n.advertise<sensor_msgs::PointCloud2>("/velodyne_points", 256);

    image_transport::ImageTransport it(n);
    image_transport::Publisher pub_image_left = it.advertise("/camera/left/image_raw", 256);
    image_transport::Publisher pub_image_right = it.advertise("/camera/right/image_raw", 256);
...
int main(int argc, char** argv)
{
    ...
    output_laser_odom_to_init=output_path+"/laser_odom_to_init/data/KITTI_"+sequence_number+"_odom.txt";
    fp_laser_odom_to_init = fopen(output_laser_odom_to_init.c_str(),"w");
    output_aft_mapped_to_init=output_path+"/aft_mapped_to_init/data/KITTI_"+sequence_number+"_odom.txt";
    fp_aft_mapped_to_init = fopen(output_aft_mapped_to_init.c_str(),"w");
    output_aft_mapped_to_init_high_frec=output_path+"/aft_mapped_to_init_high_frec_file/data/KITTI_"+sequence_number+"_odom.txt";
    fp_aft_mapped_to_init_high_frec = fopen(output_aft_mapped_to_init_high_frec.c_str(),"w");
  
    //std::string calibdir=dataset_folder+"/sequences/"+sequence_number+"/calib.txt";
